...
def seed_data(app, db):
    with app.app_context():
        ...
        if not Account.query.filter_by(id='jmb-grocery').first():
            merchant = Account(id='jmb-grocery', name='TechStart Grocery', type='MERCHANT', balance=0.0, password='password123', is_admin=False)
            db.session.add(merchant)
        else:
            merchant = Account.query.get('jmb-grocery')
            merchant.name = 'TechStart Grocery'

        if not Account.query.filter_by(id='techstart-grocery').first():
            merchant2 = Account(id='techstart-grocery', name='TechStart Grocery', type='MERCHANT', balance=0.0, password='password123', is_admin=False)
